...
class PageDAO():
    ...
    def get_page_formatted(self, page_id):
        if page_id is None:
            return None
        page = self.page.find_one({'_id': page_id})
        if page:
        ...
        for index,p in enumerate(page):
	        for key,item in p.items():
	        	if type(item) in ['str','unicode']:
	        		page[index][key]=item.decode("utf-8")
        return page
    ...
    def search(self,keyword):
        rg=re.compile(".*"+keyword+".*",re.IGNORECASE)
        result=self.page.find({'entidad':{'$regex':rg}})
        pages=[]
        for item in result:
            page=item
            del page['_id']
            for index,p in enumerate(page['personas']):
                for key,item in p.items():
                    if type(item) in ['str','unicode']:
                        page['personas'][index][key]=item.decode("latin-1")
